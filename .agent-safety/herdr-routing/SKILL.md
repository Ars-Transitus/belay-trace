---
name: herdr-routing
description: Run routed implementation and review workers as separate CLI processes in Herdr panes instead of in-process subagents, with model and reasoning effort pinned per worker, belay as the return channel, one orchestrator call per round, and up to three Task lanes. For work that independently warrants routed execution, use this transport by default inside a Herdr session unless the human explicitly opts out.
---
